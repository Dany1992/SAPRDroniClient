...
public class ManageServiceImpl implements ManageService {
	...
	public Boolean addPilot(Pilot p) {
		...
		try {
			...
			logger.info(
					String.format("Class:%s-Method:%s:: Call to WebService error[%s]", clazz, method, e.toString()));
			return false;
		}
	...
	public ArrayList<String> setAndActiveSaprs(String[] listSapr) {
		...
		// la lista conentene i sapr che eventualmente non verranno attivati per
		// un errore nel WS
		...
		for (String idSapr : listSapr) {
			...
			try {
				// se la chiamata a WS per attivare il SAPR va male metto nella
				// lista il codice
				// del sapr cosi mostro a video quali sapr non sono stati
				// attivati per qualche
				// errore
	...
	public ResponseSapr getSapr(int idSapr) {
		ResponseSapr resp = null;
		try {
			resp = service.selectSapr(idSapr);
		} catch (Exception_Exception e) {
			e.printStackTrace();
		}
		return resp;
	...
	public ResponseDevice getDevice(int idDevice) {
		ResponseDevice resp = null;
		try {
			resp = service.getDevice(idDevice);
		} catch (Exception_Exception e) {
			e.printStackTrace();
		}
		return resp;
